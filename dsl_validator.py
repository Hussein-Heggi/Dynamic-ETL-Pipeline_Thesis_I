            ...
            if p_name in registry_params:
             ...
             # type check 
                if expected_type == "string" and not isinstance(p_value, str):
                    errors.append(f"Feature {i} ('{name}'): Parameter '{p_name}' must be a string, but got {type(p_value)}.")
                elif expected_type == "int" and not isinstance(p_value, int):
                    errors.append(f"Feature {i} ('{name}'): Parameter '{p_name}' must be an integer, but got {type(p_value)}.")
